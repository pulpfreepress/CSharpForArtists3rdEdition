...
namespace Engines {
	...
	public class Engine {
		...
		public string[] CheckEngine() {
			...
			if (_oilPump.IsWorking)
				status_messages.Append("OilPump -> Working,");
			else status_messages.Append("OilPump -> Falty,");

			if (_fuelPump.IsWorking)
				status_messages.Append("FuelPump -> Working,");
			else status_messages.Append("FuelPump -> Falty,");

			if (_compressor.IsWorking)
				status_messages.Append("Compressor -> Working,");
			else status_messages.Append("Compressor -> Falty,");

			if (_temperatureSensor.IsWorking)
				status_messages.Append("TemperatureSensor -> Working,");
			else status_messages.Append("TemperatureSensor -> Falty,");

			if (_oxygenSensor.IsWorking)
				status_messages.Append("OxygenSensor -> Working");
